...
class WordFrequencyManager(TFExercise):
    ...
    def sorted(self):
        """Return frequencies as a sorted list of tuples that are ordered by frequency.
        Each tuple contains (word, frequency).
        Returns:
            Sorted list of frequencies as tuples
        """
        # Sort with item's value (item[1]) as the key
        return sorted(self._word_freqs.items(), key=lambda item: item[1], reverse=True)
